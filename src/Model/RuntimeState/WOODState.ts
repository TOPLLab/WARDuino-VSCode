...
import { WOODDumpResponse } from './WOODDumpResponse';
import { HexaStateMessages } from './HexaStateMessages';
...
export class WOODState {
    ...
    private serializeCallstack(stateMsgs: HexaStateMessages): void {
        ...
        const ws = this;
        let frames = this.woodResponse.callstack.map(f => f.serialize());
    ...
    static deserialize(line: string) {
        const trimmed = line.trimEnd();
        const wr: WOODDumpResponse = JSON.parse(trimmed);
        //TODO should be the callstack which deserializes the frames 
        // but the callstack is not yet an object
        wr.callstack = wr.callstack?.map((frame) => new Frame(frame));
