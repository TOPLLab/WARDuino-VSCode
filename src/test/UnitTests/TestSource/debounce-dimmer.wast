(module
  ...
  (import "env" "millis" (func $env.millis (type $t6)))
  ...
  (global $lastTimeDebounce (mut i32) (i32.const 0))
  (global $debounceDelay (mut i32) (i32.const 500))

  (func $debounceGuard (type $t6)
    (local $time i32)

    call $env.millis
    local.tee $time
    global.get $lastTimeDebounce
    i32.sub
    global.get $debounceDelay 
    i32.gt_u
    (if
      (then
        local.get $time
        global.set $lastTimeDebounce
        i32.const 1
        return))
    i32.const 0
    return)
  ...
  (func $decrease (type $t4) ;; Red button pin 25
    (if (call $debounceGuard)
      (then
        i32.const -127
        global.set $delta)))

  (func $increase (type $t4) ;; black button pin 33
    (if (call $debounceGuard)
      (then
        i32.const 127
        global.set $delta)))
